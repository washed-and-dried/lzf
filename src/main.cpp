...
int main(int argc, char** argv) {
    ...
    // print the separator
    MOVE_DOWN_START(1)
    printf(""); // FIXME: limit according to width
    MOVE_UP_START(1)
    ...
    while (read(STDIN_FILENO, &ch, 1) == 1) {
        switch (ch) {
            ...
            case '\r': { // \n and \r since its byte-by-byte input in raw mode
                ...
                // first we move two lines down:- one for input and one for separator
                MOVE_DOWN_START(2)
                ...
                // FIXME: MAJOR MAJOR MAJOR ISSUE.
                // ASS SOON ASS A LOOP IS INTRODUCED. THE I/O DOESN"T WORK!!!!!!!!!!!!!!
                int i = 0;
                while (i < 5) {
                    printf("HELLO\n");
                    i++;
